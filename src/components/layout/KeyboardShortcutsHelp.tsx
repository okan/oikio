  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden focus:outline-none border border-stone-200 data-[state=open]:animate-modal-in data-[state=closed]:animate-modal-out">
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  {shortcut.keys.map((key, keyIndex) => (
                    <kbd
                      key={keyIndex}
                      className="px-2 py-1 bg-stone-100 text-stone-600 text-xs font-medium rounded border border-stone-200"
                    >
                      {key}
                    </kbd>
                  ))}
